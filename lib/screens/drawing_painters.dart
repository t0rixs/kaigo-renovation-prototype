...
class WindowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final erase = Paint()
      ...
      ..strokeWidth = 10;
    final stroke = Paint()
      ..color = selected ? selectionColor : const Color(0xFF24738D)
      ..strokeWidth = 3;
    if (horizontal) {
      final wall = edge == WallEdge.top ? 0.0 : size.height;
      final inward = edge == WallEdge.top ? 1.0 : -1.0;
      final first = wall + inward * 4;
      final second = wall + inward * 10;
      canvas.drawLine(Offset(0, wall), Offset(size.width, wall), erase);
      canvas.drawLine(Offset(0, first), Offset(size.width, first), stroke);
      canvas.drawLine(Offset(0, second), Offset(size.width, second), stroke);
      canvas.drawLine(Offset(0, first), Offset(0, second), stroke);
      canvas.drawLine(
        Offset(size.width, first),
        Offset(size.width, second),
        stroke,
      );
    } else {
      final wall = edge == WallEdge.left ? 0.0 : size.width;
      final inward = edge == WallEdge.left ? 1.0 : -1.0;
      final first = wall + inward * 4;
      final second = wall + inward * 10;
      canvas.drawLine(Offset(wall, 0), Offset(wall, size.height), erase);
      canvas.drawLine(Offset(first, 0), Offset(first, size.height), stroke);
      canvas.drawLine(Offset(second, 0), Offset(second, size.height), stroke);
      canvas.drawLine(Offset(first, 0), Offset(second, 0), stroke);
      canvas.drawLine(
        Offset(first, size.height),
        Offset(second, size.height),
        stroke,
      );
    }
  }
